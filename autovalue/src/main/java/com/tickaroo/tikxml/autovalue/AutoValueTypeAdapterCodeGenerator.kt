...
fun generateValueHolder(annotatedClass: AutoValueAnnotatedClass, elementUtils: Elements): TypeSpec =
  TypeSpec.classBuilder(annotatedClass.valueHolderClassName)
    .addJavadoc("This class was generated by ${TikXmlAutoValueExtension::class.java.simpleName} for the @AutoValue annotated\n")
...
fun generateTypeAdapter(annotatedClass: AutoValueAnnotatedClass) =
  TypeSpec.classBuilder(annotatedClass.autoValueClass.simpleName.toString() + TypeAdapter.GENERATED_CLASS_SUFFIX)
    ...
    .addSuperinterface(
      ParameterizedTypeName.get(ClassName.get(TypeAdapter::class.java), ClassName.get(annotatedClass.autoValueClass.asType())))
    .apply {
      // read Xml
      val reader = "reader"
      val config = "config"
      val isGenericList = "isGenericList"
      val value = "value"
      ...
      addMethod(MethodSpec.methodBuilder("fromXml")
        ...
        .addStatement("\$T $value = (\$T) $config.getTypeAdapter(\$T.class).fromXml($reader, $config, $isGenericList)", valueHolderClass,
          valueHolderClass, valueHolderClass)
        .addCode("return new \$T(",
          ClassName.get(annotatedClass.packageName, "AutoValue_" + annotatedClass.autoValueClass.simpleName))
        .apply {
    ...
    .apply {
      ...
      addMethod(MethodSpec.methodBuilder("toXml")
        ...
        .addStatement("$config.getTypeAdapter(\$T.class).toXml($writer, $config, $tmpValue, $overridingXmlElementTagName)",
          valueHolderClass)
        .build()
      )
    }
...
fun rewriteAnnotation(annotatedMethod: AnnotatedMethod<*>, elements: Elements): AnnotationSpec =
  when (annotatedMethod) {
    ...
    is AnnotatedMethod.PropertyElementMethod -> {
      val annotation = annotatedMethod.annotation
      val converterQualifiedName =
        PropertyElementConverterChecker().getQualifiedConverterName(annotatedMethod.element, annotation)
