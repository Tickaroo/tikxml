 ...
 */
public class DocumentAutoValueTest {
  ...
  public void skipElementWithAttributesWithPath() throws IOException {
    TikXml xml = new TikXml.Builder().exceptionOnUnreadXml(false).build();
    Document document =
        xml.read(TestUtils.sourceForFile("element_with_tag_with_attributes_to_skip_with_path.xml"),
            Document.class);
    Assert.assertNotNull(document);
    Assert.assertNotNull(document.image());
    ...
    Document
        document2 = xml.read(TestUtils.sourceFrom(xmlStr), Document.class);
    Assert.assertEquals(document, document2);
  }
}
