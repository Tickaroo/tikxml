 ...
 */
@Xml
class OrganisationConstructor(
        @PropertyElement name: String,
        @param:PropertyElement val address: String?
) : WriterConstructor(name) {

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is OrganisationConstructor) return false

        val that = other as OrganisationConstructor?
