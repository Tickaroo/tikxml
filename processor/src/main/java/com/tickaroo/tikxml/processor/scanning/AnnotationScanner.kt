 ...
 * @since 1.0
 */
class AnnotationScanner(protected val elementUtils: Elements, protected val typeUtils: Types,
      ...
      if (it.isEmptyConstructorWithMinimumPackageVisibility()) {
        ...
        if (foundXmlAnnotation == constructor.parameters.size) {
          ...
          if (!constructor.hasMinimumPackageVisibilityModifiers()) {
            throw ProcessingException(constructor,
          ...
          if (annotatedConstructor != null) {
            throw ProcessingException(annotatedConstructor,
          ...
          throw ProcessingException(constructor,
            "The constructor $constructor in class ${annotatedClass.qualifiedClassName} contains a mix of TikXml annotated parameters and not annotated parameters. " +
        ...
        if (currentElement.getAnnotation(Xml::class.java) == null) {
          throw ProcessingException(currentElement,
            "The class ${annotatedClass.qualifiedClassName} should be annotated with @${Xml::class.simpleName}. This is an internal error. Please file an issue on github: https://github.com/Tickaroo/tikxml/issues") // Should be impossible
        }
   ...
   */
  @Throws(ProcessingException::class) fun checkGetter(field: Field, methodsMap: Map<String, ExecutableElement>,
    constructorAnnotatedField: Boolean): ExecutableElement {
    ...
    if (getter == null) {
      if (constructorAnnotatedField) {
        ...
        throw ProcessingException(element, "The constructor parameter '${element}' "
            + "in constructor $constructor in class ${(constructor.enclosingElement as TypeElement).qualifiedName} "
    ...
    if (getter.isProtected() || getter.isPrivate() || (getter.isDefaultVisibility() && !getter.isSamePackageAs(element,
        elementUtils))) {
      throw ProcessingException(element, "The getter method '$getter' for field '${element.simpleName}' "
          + "in class ${element.getSurroundingClassQualifiedName()} "
   ...
   */
  private inline fun fillAnnotatedClass(annotatedClass: AnnotatedClass, it: VariableElement,
    checkAccessPolicyOrDeferGetterSetterCheck: (VariableElement, Field) -> Boolean): Field? {
    ...
    when (field) {
      ...
      is PolymorphicListElementField -> {
        ...
        for ((xmlElementName, typeMirror) in field.typeElementNameMatcher) {
          val sub = PolymorphicSubstitutionListField(field.element, typeMirror, field.accessResolver, xmlElementName,
      ...
      is PolymorphicElementField -> {
        ...
        for ((xmlElementName, typeMirror) in field.typeElementNameMatcher) {
          val sub =
            PolymorphicSubstitutionField(field.element, typeMirror, field.accessResolver, xmlElementName, field.typeMirror)
          field.substitutions.add(sub)
   ...
   */
  private fun findMethodForField(fieldName: String, methodNamePrefix: String,
