...
import com.tickaroo.tikxml.processor.utils.getSuperClasses
...
import com.tickaroo.tikxml.processor.utils.isConstructor
import com.tickaroo.tikxml.processor.utils.isDefaultVisibility
import com.tickaroo.tikxml.processor.utils.isEmptyConstructor
import com.tickaroo.tikxml.processor.utils.isEmptyConstructorWithMinimumPackageVisibility
import com.tickaroo.tikxml.processor.utils.isField
import com.tickaroo.tikxml.processor.utils.isGetterMethodWithMinimumPackageVisibility
import com.tickaroo.tikxml.processor.utils.isMethodWithOneParameterOfType
import com.tickaroo.tikxml.processor.utils.isParameterlessMethod
...
import com.tickaroo.tikxml.processor.utils.isSetterMethodWithMinimumPackageVisibility
...
class AnnotationScanner(
    ...
    private fun doScan(annotatedClass: AnnotatedClass, currentElement: TypeElement): Boolean {
        ...
        val checkAccessPolicyOrDeferGetterSetterCheck =
            fun(element: VariableElement, field: Field): Boolean {
                if (element.hasMinimumPackageVisibilityModifiers()) {
                    field.accessResolver =
                        FieldAccessResolver.MinPackageVisibilityFieldAccessResolver(field.element)
                    return true
                } else {
        ...
        currentElement.enclosedElements.forEach {
            ...
            if (it.isEmptyConstructorWithMinimumPackageVisibility()) {
                ...
                constructor.parameters.forEach {
                    val mappableField = fillAnnotatedClass(
                        annotatedClass,
                        it,
                        checkAccessPolicyOrDeferGetterSetterCheck
                    )
                ...
                if (foundXmlAnnotation == constructor.parameters.size) {
                    ...
                    if (!constructor.hasMinimumPackageVisibilityModifiers()) {
                        throw ProcessingException(
                            constructor,
                            "The constructor $constructor in class ${annotatedClass.qualifiedClassName} is annotated with TikXml annotations and therefore must have at least package visibility to be able to be invoked from TikXml"
                        )
                    }
                    ...
                    if (annotatedClass.inheritance) {
                        ...
                        annotatedClass.element.getSuperClasses(typeUtils).flatMap {
                            it.enclosedElements.filter { method -> method.isGetterMethodWithMinimumPackageVisibility() }
                    ...
                    if (annotatedConstructor != null) {
                        throw ProcessingException(
                            ...
                            "Only one constructor with TikXml annotated parameters is allowed but found multiple constructors with annotated parameters in class ${annotatedClass.qualifiedClassName}: 1) $annotatedConstructor 2) $constructor"
                        )
                    }
                    ...
                    throw ProcessingException(
                        constructor,
                        "The constructor $constructor in class ${annotatedClass.qualifiedClassName} contains a mix of TikXml annotated parameters and not annotated parameters. " + "That is not allowed! Either annotate all parameters or none of them"
                    )
                ...
                if (currentElement.getAnnotation(Xml::class.java) == null) {
                    throw ProcessingException(
                        currentElement,
                        "The class ${annotatedClass.qualifiedClassName} should be annotated with @${Xml::class.simpleName}. This is an internal error. Please file an issue on github: https://github.com/Tickaroo/tikxml/issues"
                    ) // Should be impossible
                }

                val field = fillAnnotatedClass(
                    annotatedClass,
        ...
        if (!constructorFound) {
            throw ProcessingException(
                ...
                "${annotatedClass.qualifiedClassName} " + "must provide an empty (parameterless) constructor with minimum default (package) visibility"
            )
        }
        ...
        if (annotatedConstructor != null) {
            if (annotatedFields > 0) {
                throw ProcessingException(
                    annotatedConstructor,
                    "${annotatedClass.qualifiedClassName} has TikXml annotated fields AND an annotated constructor $annotatedConstructor. That is not allowed! Either annotate fields or a constructor (but not a mix of both)"
                )
            }
            ...
            annotatedConstructorFields.forEach {
                ...
                it.accessResolver =
                    FieldAccessResolver.ConstructorAndGetterFieldAccessResolver(it.element, getter)
            ...
            fieldWithMethodAccessRequired.forEach {
                ...
                // Set access policy
                it.accessResolver =
                    FieldAccessResolver.GetterSetterFieldAccessResolver(getter, setter)
    ...
    @Throws(ProcessingException::class)
    fun checkGetter(
        field: Field, methodsMap: Map<String, ExecutableElement>, constructorAnnotatedField: Boolean
    ): ExecutableElement {
        ...
        if (getter == null && element.asType().isBoolean()) {
            getter = findGetterForField(element, nameWithoutHungarian, "is", methodsMap)
                ?: findGetterForHungarianField(element, elementName, "is", methodsMap)
                        ?: findGetterForHungarianFieldUpperCase(
                    element,
                    elementName,
                    "is",
                    methodsMap
        ...
        if (getter == null) {
            if (constructorAnnotatedField) {
                ...
                throw ProcessingException(
                    element,
                    "The constructor parameter '${element}' " + "in constructor $constructor in class ${(constructor.enclosingElement as TypeElement).qualifiedName} " + "is annotated with a TikXml annotation. Therefore a getter method " + "with minimum package visibility " + "with the name ${
                        bestMethodName(
                            elementName,
                            "get"
                        )
                ...
                throw ProcessingException(
                    element,
                    "The field '${element.simpleName}' " + "in class ${(element.enclosingElement as TypeElement).qualifiedName} " + "has private or protected visibility. Hence a corresponding getter method must be provided " + "with minimum package visibility (or public visibility if this is a super class in a different package) " + "with the name ${
                        bestMethodName(
                            elementName,
                            "get"
                        )
        ...
        if (!getter.isParameterlessMethod()) {
            throw ProcessingException(
                element,
                "The getter method '$getter' for field '${element.simpleName}'" + "in class ${element.getSurroundingClassQualifiedName()} " + "must be parameterless (zero parameters)."
            )
        }

        if (getter.isProtected() || getter.isPrivate() || (getter.isDefaultVisibility() && !getter.isSamePackageAs(
                element, elementUtils
            ))
        ) {
            throw ProcessingException(
                element,
                "The getter method '$getter' for field '${element.simpleName}' " + "in class ${element.getSurroundingClassQualifiedName()} " + "must have minimum package visibility (or public visibility if this is a super class in a different package)"
            )
        }
    ...
    fun checkSetter(field: Field, methodsMap: Map<String, ExecutableElement>): ExecutableElement {
        ...
        val setter = findMethodForField(nameWithoutHungarian, "set", methodsMap)
            ...
            ?: throw ProcessingException(
                element,
                "The field '${element.simpleName}' " + "in class ${(element.enclosingElement as TypeElement).qualifiedName} " + "has private or protected visibility. Hence a corresponding setter method must be provided " + "with the name ${
                    bestMethodName(
                        elementName,
                        "set"
                    )
                }(${element.asType()}) and " + "minimum package visibility (or public visibility if this is a super class in a different package). " + "Unfortunately, there is no such setter method. Please provide one!"
            )

        if (!setter.isMethodWithOneParameterOfType(element.asType(), typeUtils)) {
            throw ProcessingException(
                element,
                "The setter method '$setter' for field '${element.simpleName}' " + "in class ${element.getSurroundingClassQualifiedName()} " + "must have exactly one parameter of type '${element.asType()}'"
            )
        }

        if (setter.isProtected() || setter.isPrivate() || (setter.isDefaultVisibility() && !setter.isSamePackageAs(
                element, elementUtils
            ))
        ) {
            throw ProcessingException(
                element,
                "The setter method '$setter' for field '${element.simpleName}' " + "in class ${element.getSurroundingClassQualifiedName()} " + "must have minimum package visibility (or public visibility if this is a super class in a different package)"
            )
        }
     ...
     */
    private inline fun fillAnnotatedClass(
        annotatedClass: AnnotatedClass,
        it: VariableElement,
        checkAccessPolicyOrDeferGetterSetterCheck: (VariableElement, Field) -> Boolean
    ): Field? {
        ...
        if (field != null) {
            ...
            if (textContentField != null) {
                ...
                throw ProcessingException(
                    it,
                    "Field '$it' is marked as TextContent and another xml element like @${Attribute::class.simpleName}, @${PropertyElement::class.simpleName} or @${Element::class.simpleName} at the same time which is not allowed. A field can only be exactly one of those types."
                )
            }
    ...
    private fun addFieldToAnnotatedClass(annotatedClass: AnnotatedClass, field: NamedField): Unit {
        when (field) {
            ...
            is AttributeField -> annotatedClass.addAttribute(
                field, PathDetector.getSegments(field.element)
            )
            ...
            is PolymorphicListElementField -> {
                ...
                for ((xmlElementName, typeMirror) in field.typeElementNameMatcher) {
                    val sub = PolymorphicSubstitutionListField(
                        field.element,
                        typeMirror,
                        field.accessResolver,
                        xmlElementName,
                        field.genericListTypeMirror
                    )
            ...
            is PolymorphicElementField -> {
                ...
                for ((xmlElementName, typeMirror) in field.typeElementNameMatcher) {
                    val sub = PolymorphicSubstitutionField(
                        field.element,
                        typeMirror,
                        field.accessResolver,
                        xmlElementName,
            ...
            is XmlChildElement -> annotatedClass.addChildElement(
                field, PathDetector.getSegments(field.element)
            )

            else -> throw IllegalArgumentException(
                "Oops, unexpected element type $field. This should never happen. Please fill an issue here: https://github.com/Tickaroo/tikxml/issues"
            )
        }
    }
    ...
    private fun findGetterForField(
        ...
        val method =
            findMethodForField(fieldName, methodNamePrefix, setterAndGetters) ?: return null
     ...
     */
    private fun findMethodForHungarianField(
        fieldName: String,
        methodNamePrefix: String,
        setterAndGetters: Map<String, ExecutableElement>
    ): ExecutableElement? {
    ...
    private fun findGetterForHungarianField(
        fieldElement: VariableElement,
        fieldName: String,
        methodNamePrefix: String,
        setterAndGetters: Map<String, ExecutableElement>
    ): ExecutableElement? {
        val method = findMethodForHungarianField(fieldName, methodNamePrefix, setterAndGetters)
            ?: return null
        return if (typeUtils.isSameType(method.returnType, fieldElement.asType())) method else null
    }
     ...
     * If field name was mFoo this method checks for a method called setMFoo()
     */
    private fun findMethodForHungarianFieldUpperCase(
        fieldName: String,
        methodNamePrefix: String,
        setterAndGetters: Map<String, ExecutableElement>
    ): ExecutableElement? {
        ...
        if (fieldName.length > 1 && fieldName.matches(Regex("m[A-Z].*"))) {
            ...
            // M in upper case
            val hungarianMethodName =
                methodNamePrefix + Character.toUpperCase(fieldName[0]) + fieldName.substring(1)
            return setterAndGetters[hungarianMethodName]
        }
    ...
    private fun findGetterForHungarianFieldUpperCase(
        fieldElement: VariableElement,
        fieldName: String,
        methodNamePrefix: String,
        setterAndGetters: Map<String, ExecutableElement>
        ...
        val method =
            findMethodForHungarianFieldUpperCase(fieldName, methodNamePrefix, setterAndGetters)
                ?: return null
        return if (typeUtils.isSameType(method.returnType, fieldElement.asType())) method else null
    }
