...
class CodeGeneratorHelper(
  ...
  fun generateAttributeBinders(currentElement: XmlElement): CodeBlock {
    ...
    for ((xmlElementName, attributeField) in currentElement.attributes) {
      ...
      fromXmlMethodBuilder.addCode(
        assignViaTypeConverterOrPrimitive(attributeField.element, AssignmentType.ATTRIBUTE, attributeField.accessResolver,
          attributeField.converterQualifiedName))
  ...
  fun generateNestedChildElementBinder(element: XmlElement): TypeSpec {
    ...
    if (element.hasAttributes()) {
      val attributeMapType =
        ParameterizedTypeName.get(ClassName.get(HashMap::class.java), ClassName.get(String::class.java), attributeBinderType)
  ...
  fun ignoreAttributes() = CodeBlock.builder()
    ...
    .addStatement("throw new \$T(\"Unread attribute '\"+ attributeName +\"' at path \"+ $readerParam.getPath())",
  ...
  fun writeAttributesAsXml(currentElement: XmlElement): CodeBlock {
    ...
    for ((_, attributeField) in currentElement.attributes) {
      builder.add(
        writeAttributeViaTypeConverterOrPrimitive(attributeField.name, attributeField.element, attributeField.accessResolver,
          attributeField.converterQualifiedName))
    }
  ...
  fun writeAttributeViaTypeConverterOrPrimitive(
    ...
    val codeWriterFormat =
      "$writerParam.$xmlWriterMethod(\"$attributeName\", $tikConfigParam.getTypeConverter(%s.class).write($resolvedGetter))"
  ...
  fun writeDelegateToTypeAdapters(type: TypeMirror, accessResolver: FieldAccessResolver, overridingXmlElementName: String?) =
    CodeBlock.builder().addStatement(
      "$tikConfigParam.getTypeAdapter(\$T.class).toXml($writerParam, $tikConfigParam, ${accessResolver.resolveGetterForWritingXml()}, ${if (overridingXmlElementName == null) "null" else "\"$overridingXmlElementName\""})",
      ClassName.get(type))
      .build()
    ...
    CodeBlock.builder()
      .apply {
        ...
        orderdByInheritanceHierarchy.forEachIndexed { i, nameMatcher ->
          ...
          addStatement("$tikConfigParam.getTypeAdapter(\$T.class).toXml($writerParam, $tikConfigParam, (\$T) $variableName, \$S)",
