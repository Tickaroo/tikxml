      ...
      for (namespace in xmlAnnotation.writeNamespaces) {
        ...
        if (XmlCharacters.containsXmlCharacter(namespace))
          throw ProcessingException(element,
            "@${Xml::class.simpleName} annotated class $simpleClassName contains an illegal namespace definition $namespace. The following characters are not allowed: < > \" ' to be used in a namespace definition")
        ...
        else throw ProcessingException(element,
          "@${Xml::class.simpleName} annotated class $simpleClassName contains an illegal namespace definition: $namespace because it contains more than 1 equals sign (=) character")
